...
func Command(builder Builder) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			ctx := cmd.Context()
			labeler, err := builder.BuildLabeler(ctx)
			if err != nil {
				return err
			}

			return label(ctx, cmd.OutOrStdout(), cmd.ErrOrStderr(), args, labeler)
		},
	}
...
func label(ctx context.Context, stdout io.Writer, stderr io.Writer, args []string, labeler Labeler) error {
	if len(args) == 0 {
	...
	if len(changes) == 0 && listFlag {
		for _, resource := range resources {
			labels, err := labeler.GetAgentLabels(ctx, resource)
			if err != nil {
	...
	for _, resource := range resources {
		labels, err := labeler.ApplyAgentLabels(ctx, resource, labels, overwriteFlag)
		if err != nil {
...
func newLabels(changes []*labelChange) (map[string]string, error) {
