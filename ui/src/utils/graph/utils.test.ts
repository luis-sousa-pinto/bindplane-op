...
import { formatMetric, getMetricForNode, getNodesAndEdges, Page, truncateLabel, updateMetricData } from "./utils";
...
describe("getMetricForNode", () => {
  ...
  it("should return the correct metric", () => {
    ...
    expect(getMetricForNode("source/source0/processors", metrics, "metrics")).toEqual(
      metrics[4],
    );

    // It finds nothing because the nodeID is for a source with no metrics metrics
    expect(getMetricForNode("source/source0", metrics, "metrics")).toBeUndefined();

    // It finds the log_data_size metric for source0's processors
    expect(getMetricForNode("source/source0/processors", metrics, "logs")).toEqual(
      metrics[1],
    );
  });
  ...
  it("should handle sources with both logs and metrics", () => {
    ...
    expect(getMetricForNode("source/source2/processors", metrics, "logs")).toEqual(
      metrics[2],
    );
  });
});
...
describe("formatMetric", () => {
  it("converts a metric to a human readable string", () => {
    expect(
      formatMetric({ value: 10, unit: "B/s" }, "/s")
    ).toEqual("10 B/s");
  });

  it("converts to greater units if needed", () => {
    expect(
      formatMetric({ value: 2048, unit: "KiB/s" }, "/s")
    ).toEqual("2 MiB/s");
  });

  it("converts to the requested rate", () => {
    expect(
      formatMetric({ value: 1024, unit: "MiB/s" }, "/m")
    ).toEqual("60 GiB/m");

    expect(
      formatMetric({ value: 1024, unit: "MiB/s" }, "/h")
    ).toEqual("3.5 TiB/h");
  });
});
